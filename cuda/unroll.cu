...
__global__ void readOffsetUnroll2(float *A, float *B, float *C, const int n,
                                  int offset)
{
	//NOTICE: here we do not assign each thread to do the two continuous elements' addition
	//Instead each thread do i-th and i+blockDim.x-th, which will bring better memory coalesce access
