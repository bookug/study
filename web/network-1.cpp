...
public:	
	...
	void send(unsigned offset, unsigned cnt)	//send from head+offset, cnt in total
	{
		...
		while(cnt > 0)
		{
			SendFRAMEPacket(this->q[i]->fp, this->q[i]->length);
			i = (i + 1) % MAX_QUEUE_LEN;
			cnt--;
		}
	}
...
int stud_slide_window_stop_and_wait(char *pBuffer, int bufferSize, UINT8 messageType)
{
	...
	case MSG_TYPE_SEND:
		...
		if(queue.count() <= WINDOW_SIZE_STOP_WAIT)
			SendFRAMEPacket(pBuffer, bufferSize);
		break;
	case MSG_TYPE_RECEIVE:
		tack = ((frame*)pBuffer)->head.ack;
		if(ack == queue.top()->fp->head.seq)	//both are big-endian
		{
			queue.pop();
			if(!queue.isEmpty())
			{
				p = queue.top();
				SendFRAMEPacket(p->fp, p->length);
			}
		}
		break;
	case MSG_TYPE_TIME_OUT:
		tseq = *(UINT32*)pBuffer;
		p = queue.top();
...
int stud_slide_window_back_n_frame(char *pBuffer, int bufferSize, UINT8 messageType)
{
	...
	case MSG_TYPE_RECEIVE:
		tack = ((frame*)pBuffer)->head.ack;
		int ret = queue.find(tack, WINDOW_SIZE_BACK_N_FRAME);		//both are big-endian
	...
	case MSG_TYPE_TIME_OUT:
		...
		int ret = queue.find(tseq, WINDOW_SIZE_BACK_N_FRAME);
		if(ret >= 0)
		{
...
int stud_slide_window_choice_frame_resend(char *pBuffer, int bufferSize, UINT8 messageType)
{
	...
	case MSG_TYPE_SEND:
		...
		break;
	case MSG_TYPE_RECEIVE:			
		frame_kind kind = ntohl(((frame*)pBuffer)->head.kind);
		if(kind == ack)
		{
			tack = ((frame*)pBuffer)->head.ack;
			int ret = queue.find(ack, WINDOW_SIZE_CHOICE_FRAME_RESEND);
			if(ret >= 0)
			{
				unsigned t = queue.count(), i;
				unsigned num = ret + 1;
		...
		else if(kind == nak)
		{
			...
			int ret = queue.find(tseq, WINDOW_SIZE_CHOICE_FRAME_RESEND);
			queue.send(ret, 1);
		}
		break;
	default:
